...
/*nf_conntrack 随机数，用于计算hash_conntrack 时使用  */
...
/* 当conntrack会话表满后会进入该函数，删除半连接表项，为新连接腾出空间 */
...
static noinline int early_drop(struct net *net, unsigned int hash)
{
	...
	for (i = 0; i < net->ct.htable_size; i++) {
		/* NOT UNDERSTAND !!!
		这里不明白作者为什么需要找到一个符合条件的ct后不break，
		而是找到hash链最后一个ct，对其进行操作,难道是因为新添加的ct在hash头上添加的，
		但是感觉没有必要吧，这样效率相对很低*/
